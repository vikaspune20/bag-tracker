...
export const Trips = () => {
  ...
  const [bagsData, setBagsData] = useState<any[]>([
    { tagNumber: "", weight: "", description: "", image: null, deviceId: "" },
  ]);
  ...
  const handleEndTrip = async (tripId: string) => {
    if (
      !confirm(
        "End this trip? Attached tracking devices will be released and can be reused.",
      )
    )
      return;
    try {
  ...
  const handleCreateTrip = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      alert(
        `Error creating trip: ${error.response?.data?.message || error.message}`,
      );
    }
  };
              ...
              >
                ...
                <div className="bg-gradient-to-r from-airline-dark to-airline-blue px-5 pt-4 pb-5 text-white">
                  ...
                  <div className="flex items-center justify-between mt-4">
                    ...
                    <div className="text-right">
                      <p className="text-2xl font-black tracking-tight">
                        {trip.destinationAirport?.split(" ")[0]}
                      </p>
                      <p className="text-xs text-blue-200 mt-0.5">
                        {trip.arrivalDateTime
                          ? format(
                              new Date(trip.arrivalDateTime),
                              "MMM d, HH:mm",
                            )
                          : "—"}
                      </p>
                ...
                <div className="px-5 py-4 flex-1 space-y-2">
                  ...
                  {trip.bags?.some((b: any) => b.device) ? (
                    <p className="text-[11px] text-airline-blue font-semibold">
                      Tracking active
                    </p>
                  ) : (
                    <p className="text-[11px] text-gray-400 italic">
                      No tracking device linked
                    </p>
                  )}
                </div>
                ...
                <div className="px-5 pb-5 space-y-2">
                  ...
                  {trip.endedAt ? (
                    <div className="w-full py-2 text-center text-xs font-semibold text-gray-500 bg-gray-50 rounded-xl">
                      Trip ended ·{" "}
                      {format(new Date(trip.endedAt), "MMM d, HH:mm")}
                    </div>
                  ) : (
                    <button
            ...
            <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-2xl overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                ...
                <button
                  ...
                  className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-200 text-xl leading-none"
                >
                  &times;
                </button>
              </div>
                ...
                <form onSubmit={handleCreateTrip} className="space-y-6">
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center pb-2 border-b border-gray-100">
                      <h4 className="font-bold text-gray-900 flex items-center">
                        <Briefcase
                          size={18}
                          className="mr-2 text-airline-blue"
                        />{" "}
                        Registered Bags
                      </h4>
                      <button
                        type="button"
                        onClick={handleAddBagForm}
                        className="text-sm font-bold text-airline-sky hover:text-airline-blue flex items-center"
                      >
                    ...
                    {bagsData.map((bag, idx) => (
                      ...
                      >
                        {bagsData.length > 1 && (
                          <button
                            type="button"
                            onClick={() => handleRemoveBagForm(idx)}
                            className="absolute top-4 right-4 text-gray-400 hover:text-red-500"
                          >
                            <Trash2 size={18} />
                          </button>
                        )}
                        <h5 className="text-sm font-bold text-gray-500 mb-3">
                          Bag #{idx + 1}
                        </h5>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="md:col-span-2">
                            ...
                            <select
                              value={bag.deviceId}
                              onChange={(e) =>
                                handleBagChange(idx, "deviceId", e.target.value)
                              }
                              className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-1.5 px-3 focus:ring-airline-sky sm:text-sm"
                            >
                              <option value="">
                                — No device, type a tag manually —
                              </option>
                              {devices
                                .filter(
                                  (d) =>
                                    !usedDeviceIds.has(d.deviceId) ||
                                    d.deviceId === bag.deviceId,
                                )
                                .map((d) => (
                          ...
                          <div>
                            ...
                            <input
                              ...
                              value={bag.weight}
                              onChange={(e) =>
                                handleBagChange(idx, "weight", e.target.value)
                              }
                              placeholder="45.5"
                              className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-1.5 px-3 focus:ring-airline-sky sm:text-sm"
                            />
                          ...
                          <div className="md:col-span-2">
                            ...
                            {bag.image && (
                              <img
                                src={URL.createObjectURL(bag.image)}
                                alt="bag preview"
                          ...
                          <div className="md:col-span-2">
                            ...
                            <input
                              ...
                              placeholder="Black Samsonite handle..."
                              className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-1.5 px-3 focus:ring-airline-sky sm:text-sm"
                            />
                          </div>
                        </div>
                  ...
                  <div className="flex justify-end pt-4 border-t border-gray-100 mt-6">
                    <button
                      type="submit"
                      disabled={loading}
                      className="bg-airline-blue text-white px-8 py-3 rounded-lg font-bold shadow-md hover:bg-airline-dark transition-colors flex items-center"
                    >
                      {loading ? (
